...
def cifar100_get_datasets(data, load_train=True, load_test=True, bayer=False, fold=True):
    """
    ...
    if load_test:
        if (bayer and fold):
            test_transform = transforms.Compose([
                transforms.Resize((32, 32)), bayer_filter_2(),fold_data(),
                transforms.ToTensor(),
                ai8x.normalize(args=args)
            ])
        elif (bayer):
            test_transform = transforms.Compose([
                transforms.Resize((32, 32)), bayer_filter(),
                transforms.ToTensor(),
                ai8x.normalize(args=args)
            ])
        else:
